<?php
	class Personas extends Controllers
	{
		public function __construct()
		{
			...
			// "getPermisos(2)" -> Para extraer los permisos que corresponde al modulo en el momento, para este caso es el 2 (Personas)

			getPermisos(2); // Este el Id que corresponde en la tabla de Modulos; 2 = Personas
		}
		...
		public function Personas()
		{
			...
			$data['page_title'] = "PERSONAS <small>Tienda Virtual</small>";
		...
		// Obteniene las "Personas" con el "Rol"
		...
		{
			// Esta condicion se utiliza para que usuarios que no tengan sesion no pueden visualizar las personas.
			if ($_SESSION['permisosMod']['r'])
			{
				$htmlOptions = "";
				// Se estan obteniendo los usuarios desde la base de datos				
				$arrData = $this->model->selectPersonas();
				 //dep($arrData);
				 //die();exit;
				 // Si hay Informacion, crea la estructura del "SELECT"
				 if (count($arrData) > 0)
				 {
					 for ($i=0; $i<count($arrData); $i++)
					 {
						 if ($arrData[$i]['estatus'] == 1) // Muestra que esten activo las "Personas"
						 {
							 //$htmlOptions .= '<option value = "'.$arrData[$i]['id_categoria'].'">'.$arrData[$i]['nombre'].'</option>';
							 // Se arma el "Select"
							 $htmlOptions .= "<option value = '".$arrData[$i]['id_persona']."'>".$arrData[$i]['nombre_completo']."</option>";
						 }
					 }
				 } // if (count($arrData) > 0)
