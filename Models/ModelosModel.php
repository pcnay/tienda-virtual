<?php
	class ModelosModel extends Mysql
	{
		...
		*/
		...
		public function insertModelo(string $descripcion)
		{
			$return = 0;
			$this->strDescripcion = $descripcion;
			
			// Verificar si existe el Modelo.
			$sql = "SELECT * FROM t_Modelo WHERE descripcion = '{$this->strDescripcion}'";
			$request = $this->select_all($sql);

			// Si no encontro el registro.
			if (empty($request))
			{
				// "id_modelo" no se contempla, porque se definio en la base datos como autoincrementable. 
				$query_insert = "INSERT INTO t_Modelo(descripcion) VALUES (?)";
		...
		public function selectModelo(int $idModelo)
		{
			...
			$sql = "SELECT * FROM t_Modelo WHERE id_modelo = $this->intIdModelo";
			$request = $this->select($sql); // Este método se definio en MySQL
			return $request;
		}
		
		// Actualizar un Modelo.
		public function updateModelo(int $idModelo,string $descripcion)
		{
			$this->intIdModelo = $idModelo;			
			$this->strDescripcion = $descripcion;
			
			// Verificando atraves del "nombre" que no exita						
			$sql = "SELECT id_modelo,descripcion FROM t_Modelo WHERE (descripcion = '{$this->strDescripcion}' AND id_modelo != '{$this->intIdModelo}')";
			
			$request = $this->select_all($sql);
			// Si esta vacio, por lo tanto no esta duplicado el "descripcion"
			if (empty($request))
			{
					$sql = "UPDATE t_Modelo SET descripcion=? WHERE id_modelo = $this->intIdModelo";
		...
		public function deleteModelo(int $idModelo)
		{
			...
			$this->intIdModelo = $idModelo;
				$sql = "DELETE FROM t_Modelo WHERE id_modelo = $this->intIdModelo";
